...
__all__ = [
    ...
    "build_all_shareholders",
    "get_shareholder_questions",
]
...
def load_shareholder_specs() -> list[dict]:
    ...
    for yaml_file in sorted(_REGISTRY_DIR.glob("shareholder_*.yaml")):
        try:
            ...
            specs.append(spec)
            logger.info(
                "[ShareholderAgent] Loaded shareholder: %s",
                spec.get("name", yaml_file.name),
            )
        except Exception as exc:
            logger.warning(
                "[ShareholderAgent] Error loading %s: %s", yaml_file.name, exc
            )
    return specs
...
def create_shareholder_agent(spec: dict):
    ...
    try:
        ...
        logger.debug(
            "[ShareholderAgent] Heavy LLM unavailable, falling back to analysis tier"
        )
    ...
    return Agent(
        ...
        goal=spec.get(
            "goal",
            f"Ask one question about {spec.get('investment_thesis', 'the deal')}",
        ),
...
def build_all_shareholders() -> list:
    ...
    for spec in specs:
        try:
            ...
            logger.warning(
                "[ShareholderAgent] Failed to create agent from %s: %s",
                spec.get("_file", "?"),
                exc,
            )
    return agents
...
def get_shareholder_questions(market: str, deal_summary: str) -> list[dict]:
        ...
        return [
            {
                ...
                "question": "Define shareholder personas first (J-2 in TASK_QUEUE.md).",
            }
        ]
    ...
    for spec, agent in shareholders:
        try:
            ...
            logger.info(
                "[ShareholderAgent] %s asks: %s", spec.get("name", "?"), question[:80]
            )
        except Exception as exc:
            logger.warning(
                "[ShareholderAgent] %s failed: %s", spec.get("name", "?"), exc
            )
            results.append(
                {
                    "name": spec.get("name", "Shareholder"),
