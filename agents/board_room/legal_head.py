...
class RERAComplianceTool(BaseTool):
    ...
    def _run(self, input_str: str) -> str:
        ...
        try:
            ...
            logger.info(
                "[RERAComplianceTool] developer=%s market=%s signal=%s",
                r.developer_name,
                market,
                r.compliance_signal,
            )
...
class ZoneRiskTool(BaseTool):
    ...
    def _run(self, input_str: str) -> str:
        ...
        try:
            ...
            logger.info(
                "[ZoneRiskTool] market=%s zone=%s risk_level=%s overlays=%d",
                r.market,
                r.zone,
                r.risk_level,
                len(r.overlay_risks),
...
class EncumbranceCheckTool(BaseTool):
    ...
    def _run(self, input_str: str) -> str:
        ...
        try:
            ...
            logger.info(
                "[EncumbranceCheckTool] market=%s data_source=%s regs=%d gap=%s",
                r.market,
                r.data_source,
                r.registration_count_180d,
                r.guidance_gap_pct,
            )
            return json.dumps(
                {
