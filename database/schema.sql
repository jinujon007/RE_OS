...
--   where monthly_registrations = 12-month avg registration count from kaveri
...
-- Kaveri registration stats: 12-month rolling window.
-- Records with NULL registration_date are excluded from the date-based count
-- but contribute to the raw count for markets where most records lack dates.
kaveri_stats AS (
    SELECT
        kr.micro_market_id,
