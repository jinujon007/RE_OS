...
class TestSeedListingsGenerator:
    ...
    def test_listing_has_required_fields(self):
        ...
        required = {"id", "source", "micro_market_id", "data_source", "price_psf", "locality"}
    ...
    def test_source_equals_data_source(self):
        ...
        for listing in listings:
            assert listing["source"] == "seed_estimated", f"source={listing['source']} should be seed_estimated"
    ...
    def test_deterministic_seed_produces_same_output(self):
        ...
        with patch("database.seed_listings.PROJECT_TEMPLATES", {
            ...
            for a, b in zip(batch1, batch2):
                assert a["price_psf"] == b["price_psf"], "Same seed should produce same PSF"
                assert a["bhk_config"] == b["bhk_config"], "Same seed should produce same BHK"
...
class TestSeedListingsInsert:
    def test_insert_listings_none_skipped(self):
        ...
        for l in listings:
            for c in ["source_listing_id", "source_url", "micro_market_id", "rera_project_id",
                       "property_type", "transaction_type", "bhk_config", "carpet_area_sqft",
                       "built_up_area_sqft", "super_built_up_sqft", "plot_area_sqft",
                       "listed_price", "price_psf", "monthly_rent", "security_deposit",
                       "deposit_months", "address", "listed_at", "days_on_market",
                       "is_new_launch", "is_rera_registered", "raw_rera_number", "raw_data",
