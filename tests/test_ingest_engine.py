...
class TestParsedRecord:
    ...
    def test_deterministic_hash(self):
        ...
        r2 = ParsedRecord(
            entity_type="rera_project", source_id="p2", market="Hebbal", data={"a": 1}
        )
...
class TestDataPluginBase:
    ...
    def test_validate_empty_source_id(self):
        r = ParsedRecord(
            entity_type="rera_project", source_id="", market="M", data={"k": "v"}
        )
        v = _SuccessPlugin().validate(r)
        assert v.valid is False
    ...
    def test_validate_empty_data(self):
        r = ParsedRecord(
            entity_type="rera_project", source_id="s1", market="M", data={}
        )
        v = _SuccessPlugin().validate(r)
        assert v.valid is False
    ...
    def test_validate_success(self):
        r = ParsedRecord(
            entity_type="rera_project", source_id="s1", market="M", data={"k": "v"}
        )
        v = _SuccessPlugin().validate(r)
...
class TestPluginRun:
    def test_successful_plugin_run(self):
        ...
        engine.register(_SuccessPlugin())
        with (
            patch.object(engine._writer, "write", return_value=True),
...
class TestBackoff:
    def test_backoff_values_increase_monotonically(self):
        ...
        for i in range(1, len(durations)):
            assert durations[i] >= durations[i - 1] - 0.01, (
                f"backoff not monotonic at attempt {i}"
            )
...
class TestGeocoder:
    def test_cache_hits_return_same_result(self):
        ...
        g = _Geocoder()
        with patch.object(
            g, "_call_nominatim", return_value={"lat": 13.0, "lon": 77.5}
        ):
