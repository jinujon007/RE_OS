...
def _make_mock_row(listing_id, source_url, property_type, locality, project_name):
    ...
    row.__iter__.return_value = iter(
        [listing_id, source_url, property_type, locality, project_name]
    )
    return row
...
def test_locality_validation_catches_alien_listing():
    ...
    with patch("utils.db.get_engine") as mock_eng:
        ...
        mock_conn.execute.return_value.fetchall.return_value = [
            _make_mock_row(
                "uuid-1",
                "https://example.com/some-property",
                "Apartment",
                "Whitefield",
                "Prestige SomeProject",
            ),
        ]
...
def test_locality_validation_clean_data_scores_1():
    ...
    with patch("utils.db.get_engine") as mock_eng:
        ...
        mock_conn.execute.return_value.fetchall.return_value = [
            _make_mock_row(
                "uuid-1",
                "https://example.com/yelahanka-property",
                "Apartment",
                "Yelahanka",
                "Sobha Yelahanka",
            ),
            _make_mock_row(
                "uuid-2",
                "https://example.com/vidyaranyapura",
                "Villa",
                "Vidyaranyapura",
                "Prestige Vidyaranyapura",
            ),
        ]
        result = DataQualityMonitor.locality_validation_score("Yelahanka")
...
def test_locality_validation_below_threshold_alerts():
    ...
    with patch("utils.db.get_engine") as mock_eng:
        ...
        mock_conn.execute.return_value.fetchall.return_value = [
            ...
            _make_mock_row(
                "uuid-2",
                "https://example.com/whitefield-property",
                "Villa",
                "Whitefield",
                "Prestige Whitefield",
            ),
        ]
        result = DataQualityMonitor.locality_validation_score(
            "Yelahanka", max_suspect_pct=20.0
        )
