...
def test_market_forecasts_table_exists_via_schema():
    ...
    assert "check" in migration_path.read_text().lower(), "Migration missing CHECK constraints"
...
def test_finance_head_context_contains_psf_forecast_string():
    ...
    result = ForecastResult(
        market="Yelahanka", status="ok", current_psf=6500.0,
        trend_direction="rising", forecast_6m=7100.0,
        conf_low_6m=6500.0, conf_high_6m=7700.0,
        mae_pct=3.5, data_points=6,
    )
