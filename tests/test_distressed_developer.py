...
class TestDistressedDeveloperScanner:
    ...
    def test_db_error_returns_empty(self):
        with patch("utils.distressed_developer.get_engine", side_effect=Exception("DB down")):
            from utils.distressed_developer import scan_distressed_developers
            results = scan_distressed_developers("Yelahanka")
            assert results == []
...
class TestFormatDistressAlert:
    ...
    def test_format_contains_score_and_name(self):
        from utils.distressed_developer import DistressedDeveloper, format_distress_alert
        dev = DistressedDeveloper(
            developer_name="Test Builder", market="Yelahanka",
            total_projects=3, active_projects=1, delayed_projects=2,
            avg_delay_months=12.0, incomplete_ratio=0.5, complaint_count=2,
            distress_score=0.72, alert_level="HIGH_DISTRESS",
        )
