...
class TestObsidianExport:
    ...
    def test_obsidian_export_writes_correct_frontmatter(self):
        ...
        result_obj = WeeklyDigestResult(
            market="Yelahanka", psf_delta_pct=5.25, psf_direction="up",
            new_rera_count=3, competitor_launches=[], distressed_developers=[],
            top_opportunity=None,
        )
        with patch("utils.obsidian_export.os.path.isdir", return_value=True):
            mock_open = MagicMock()
            with patch("builtins.open", mock_open):
                with patch("utils.obsidian_export.os.path.join", return_value="/fake/path.md"):
