...
def test_gcc_plugin_has_snapshot_methods():
    ...
    assert hasattr(plugin, "_get_snapshot_employers"), "GCCPlugin missing _get_snapshot_employers"
    assert hasattr(plugin, "_scan_hiring_snapshots"), "GCCPlugin missing _scan_hiring_snapshots"
    ...
    assert "snapshot_employers" in src, "GCCPlugin.run must reference snapshot employers"


def test_dc_conversions_importable_and_parser_works():
    """Assertion 3: dc_conversions module importable + parser round-trip."""
    from scrapers.dc_conversion_scraper import _parse_dc_html, run_scan, market_for_village
    html = """
    <table>
...
def test_uncallibrated_label_present_pre_calibration():
    ...
    output = str(ds)
    assert "UNCALIBRATED" in output, f"[UNCALIBRATED] label missing from str(): {output}"
