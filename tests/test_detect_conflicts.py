...
class TestConflictDetection:
    ...
    def _build_mock_row(self, agent_a="analyst", agent_b="ceo",
                        fact_a="Based on latest data, PSF is ₹6200",
                        fact_b="Based on latest data, PSF is ₹4800",
                        conf_a=0.7, conf_b=0.8, market="Yelahanka"):
        return MagicMock(
            _asdict=lambda: {
            ...
            __iter__=lambda s: iter([
                agent_a, agent_b, fact_a, fact_b, conf_a, conf_b, market,
            ]),
        )
    ...
    def test_skips_non_numeric_facts(self):
        ...
        rows = [MagicMock(
            __iter__=lambda s: iter([
                "analyst", "ceo",
                "Market is trending upward", "Demand is strong",
                0.7, 0.8, "Yelahanka",
            ]),
        )]
        self._patch_engine(rows)
    ...
    def test_conflict_keys_present(self):
        ...
        try:
            result = detect_conflicts("Yelahanka")
            assert len(result) >= 1
            expected_keys = {"market", "fact_prefix", "agent_a", "agent_b",
    ...
    def test_conflict_fact_truncated_to_500_chars(self):
        ...
        try:
            ...
            written_fact = call_kwargs.get("parameters", {}).get("fact") or \
                          list(mock_write.execute.call_args[0][1].values())[2]
