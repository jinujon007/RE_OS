...
class TestLandownerCRM:
    def test_create_landowner_returns_201(self):
        with patch("utils.db.get_engine") as mock_eng:
            ...
            mock_conn.execute.return_value.fetchone.return_value = (
                "u1", "45/2", "Devanahalli", "Venkatesh Gowda",
                "9876543210", "primary", "cold", None, None, None, None,
            )
            client = _fresh_client()
            resp = client.post("/api/landowners", json={
                "survey_no": "45/2", "market": "Devanahalli",
                "owner_name": "Venkatesh Gowda", "contact_phone": "9876543210",
    ...
    def test_create_landowner_rejects_invalid_contact_type(self):
        with patch("utils.db.get_engine"):
            ...
            resp = client.post("/api/landowners", json={
                "survey_no": "45/2", "market": "Devanahalli",
                "owner_name": "Test", "contact_type": "invalid_type",
            }, headers={"X-API-Key": "test"})
    ...
    def test_list_returns_paginated(self):
        with patch("utils.db.get_engine") as mock_eng:
            ...
            mock_conn.execute.return_value.fetchall.return_value = [
                ("u1", "45/2", "Devanahalli", "Owner1", None, "primary", "cold",
                 None, None, None, None),
            ]
            client = _fresh_client()
            resp = client.get("/api/landowners?market=Devanahalli", headers={"X-API-Key": "test"})
    ...
    def test_update_approach_status_returns_200(self):
        with patch("utils.db.get_engine") as mock_eng:
            ...
            mock_conn.execute.return_value.fetchone.side_effect = [
                ...
                ("u1", "45/2", "Devanahalli", "Venkatesh Gowda", None, "primary",
                 "warm", None, None, None, None),
            ]
            client = _fresh_client()
            resp = client.patch("/api/landowners/00000000-0000-0000-0000-000000000001", json={
    ...
    def test_discord_fires_on_mou(self):
        with patch("utils.discord_notifier.send") as mock_send:
            ...
            msg = format_landowner_update("mou", "45/2", "Devanahalli", "Venkatesh Gowda", 7500)
            assert "MOU" in msg
            assert "₹7,500" in msg
            assert len(msg) <= 500
    ...
    def test_discord_fires_on_mou_via_send_alert(self):
        with patch("utils.discord_notifier.send") as mock_send:
            ...
            result = send_landowner_alert("loi", "45/2", "Devanahalli", "Test Owner", 8000)
