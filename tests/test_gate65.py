...
def test_gate65_portfolio_endpoint_returns_projects():
    ...
    with patch("utils.db.get_engine") as mock_eng:
        ...
        mock_conn.execute.return_value.fetchall.return_value = [
            ("u1", "Project A", "Bangalore", "Yelahanka", "premium",
             500, 480, date(2020, 1, 15), date(2024, 6, 30),
             50.0, 300.0, 18.5, "delivered", "R/1", "note", datetime(2026, 6, 8)),
            ("u2", "Project B", "Bangalore", "Devanahalli", "mid_market",
             300, 290, date(2019, 3, 1), date(2023, 12, 15),
             35.0, 180.0, 15.2, "delivered", "R/2", "note", datetime(2026, 6, 8)),
            ("u3", "Project C", "Bangalore", "Yelahanka", "premium",
             200, 185, date(2018, 6, 20), date(2022, 3, 31),
             25.0, 140.0, 21.4, "delivered", "R/3", "note", datetime(2026, 6, 8)),
            ("u4", "Project D", "Bangalore", None, "luxury",
             350, 340, date(2021, 1, 10), date(2025, 6, 30),
             95.0, 620.0, 17.8, "delivered", "R/4", "note", datetime(2026, 6, 8)),
        ]
...
def test_gate65_investor_brief_section_6_non_empty():
    ...
    pkg = IntelPackage(
        ...
        module_status={}, deal_type="compare",
    )
    _Mpulse = MagicMock()
...
def test_gate65_investor_brief_section_7_market_position():
    ...
    pb = PeerBenchmarkResult(
        market="Devanahalli", as_of="2026-06-08T00:00:00",
        grade_a_count=5, avg_psf_grade_a=7200.0,
        median_absorption_pct_grade_a=60.0, avg_units_grade_a=350.0,
        lls_target_psf=7500.0, lls_vs_grade_a_pct=4.17,
