...
class TestDemandPlugin:
    def test_demand_plugin_returns_list(self):
        ...
        with patch("ingest.plugins.demand_plugin._fetch_nri_listings", return_value=[]):
            with patch("ingest.plugins.demand_plugin._check_listing_surge", return_value=None):
                with patch("ingest.plugins.demand_plugin._check_price_cuts", return_value=None):
    ...
    def test_demand_plugin_handles_network_error(self):
        ...
        plugin = DemandPlugin()
        with patch("ingest.plugins.demand_plugin._fetch_nri_listings", return_value=[]):
            with patch("ingest.plugins.demand_plugin._check_listing_surge", side_effect=Exception("DB down")):
                result = plugin.run("Yelahanka")
                assert isinstance(result, list)

    def test_demand_events_table_columns(self):
        expected_columns = {
            "id", "market", "event_type", "count", "value_cr", "source", "recorded_at",
        }
