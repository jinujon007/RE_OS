...
@pytest.fixture
def mock_db():
    with patch("utils.weekly_digest.get_engine") as w_eng, \
...
class TestGate76:
    def test_weekly_digest_build_returns_result_without_raising(self, mock_db):
        ...
        assert all(hasattr(result, f) for f in ("market", "psf_delta_pct", "psf_direction", "new_rera_count"))
    ...
    def test_monthly_digest_build_returns_result_without_raising(self, mock_db):
        ...
        with patch.object(MonthlyIntelDigest, "_generate_synthesis", return_value=""):
            ...
            assert all(hasattr(result, f) for f in ("market", "psf_mom_pct", "absorption_trend"))
    ...
    def test_format_weekly_digest_under_400_chars(self):
        ...
        result = WeeklyDigestResult(
            ...
            new_rera_count=3,
            competitor_launches=[{"developer_name": "Brigade", "project_name": "Test", "grade": "A", "units": 100}],
            distressed_developers=[{"developer_name": "DevX", "market": "Yelahanka", "distress_score": 0.75}],
