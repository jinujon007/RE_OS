...
def test_plugin_returns_parsed_records():
    with patch("ingest.plugins.dc_conversion_plugin.run_scan") as mock_scan:
        mock_scan.return_value = [
            {"application_no": "DC/001", "village": "Venkatala", "survey_no": "45/2",
             "to_use": "Residential", "from_use": "Agri", "status": "Approved"},
            {"application_no": "DC/002", "village": "Byatarayanapura", "survey_no": "10/1",
             "to_use": "Commercial", "from_use": "Agri", "status": "Pending"},
        ]
...
def test_plugin_falls_back_to_inbox():
    with patch("ingest.plugins.dc_conversion_plugin.run_scan") as mock_scan:
        mock_scan.side_effect = [Exception("live failed"), [
            {"application_no": "DC/010", "village": "Jakkur", "survey_no": "5/1", "status": "Approved"}
        ]]
        from ingest.plugins.dc_conversion_plugin import DCConversionPlugin
        from ingest.base import ParsedRecord
        plugin = DCConversionPlugin()
