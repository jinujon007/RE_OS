...
def test_a4_migration_has_response_time_s():
    ...
    with open(path) as f:
        content = f.read()
    assert "response_time_s" in content, "Migration must define response_time_s column"
    assert "board_sessions" in content, "Migration must reference board_sessions table"
