...
class TestConfigAbsorption:
    def test_config_absorption_returns_dict(self):
        ...
        with patch("intelligence.demand_intel.validate_market", return_value={
            "id": "uuid", "name": "Yelahanka", "slug": "yelahanka",
        }):
            with patch("intelligence.demand_intel.sanitize_market", return_value="Yelahanka"):
                with patch("utils.db.get_engine", return_value=mock_engine):
                    with patch.object(di, "_query_config_absorption", return_value={
                        "1BHK": 75.0, "2BHK": 60.0, "3BHK": 45.0,
                    }):
...
class TestAbsorptionTrend:
    def test_absorption_trend_returns_list(self):
        ...
        with patch("intelligence.demand_intel.validate_market", return_value={
            ...
            with patch("intelligence.demand_intel.sanitize_market", return_value="Yelahanka"):
                with patch("utils.db.get_engine", return_value=mock_engine):
                    with patch.object(di, "_query_absorption_trend", return_value=[
    ...
    def test_absorption_trend_sorted_by_month(self):
        ...
        with patch("intelligence.demand_intel.validate_market", return_value={
            ...
            with patch("intelligence.demand_intel.sanitize_market", return_value="Yelahanka"):
                with patch("utils.db.get_engine", return_value=mock_engine):
                    with patch.object(di, "_query_absorption_trend", return_value=trend_data):
                        result = di.get_absorption_trend("Yelahanka")
    ...
    def test_absorption_trend_single_month(self):
        ...
        with patch("intelligence.demand_intel.validate_market", return_value={
            ...
            with patch("intelligence.demand_intel.sanitize_market", return_value="Yelahanka"):
                with patch("utils.db.get_engine", return_value=mock_engine):
                    with patch.object(di, "_query_absorption_trend", return_value=[
                        {"month": "2026-01", "avg_absorption_pct": 50.0, "project_count": 3},
                    ]):
                        result = di.get_absorption_trend("Yelahanka")
                        assert len(result) == 1
    ...
    def test_days_on_market_by_config_returns_dict(self):
        ...
        with patch("intelligence.demand_intel.validate_market", return_value={
            ...
            with patch("intelligence.demand_intel.sanitize_market", return_value="Yelahanka"):
                with patch("utils.db.get_engine", return_value=mock_engine):
                    with patch.object(di, "_query_days_on_market_by_config", return_value={
                        "1BHK": 180.0, "2BHK": 240.0, "3BHK": 300.0,
                    }):
