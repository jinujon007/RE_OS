...
from scripts.eval_florence2 import _calculate_cer, _get_device, _measure_vram, _model_size_mb
...
class TestMeasureVram:
    ...
    def test_cuda_returns_values(self):
        ...
        with patch("scripts.eval_florence2.torch.cuda", mock), \
...
class TestModelSizeMb:
    ...
    def test_shared_parameters_deduped(self):
        ...
        p2 = self._make_param(1_400_000, 2, 100)    # Shared ptr — same as p1, should NOT double
        mock_model = MagicMock(spec=[])
        ...
        assert size < 440.0  # Should be ~438.7, not ~442.0 (would include shared weight)
...
class TestMain:
    def test_missing_images_returns_error(self):
        with patch("scripts.eval_florence2._load_images", return_value={}), \
    ...
    def test_model_load_failure_returns_error(self):
        ...
        with patch("scripts.eval_florence2._load_images", return_value=mock_images), \
             patch("scripts.eval_florence2.AutoProcessor.from_pretrained", side_effect=RuntimeError("OOM")), \
             patch("scripts.eval_florence2._cleanup_gpu"):
            from scripts.eval_florence2 import main
            assert main() == 1
