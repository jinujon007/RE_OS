...
class TestFSIWithAIZ:
    ...
    def test_aiz_no_cap_when_no_zone_row(self):
        ...
        result = calculate_fsi(
            43560, zone="R2", market="Yelahanka", _aiz_override=(None, None)
        )
    ...
    def test_aiz_no_override_uses_db_lookup(self):
        with patch(
            "utils.fsi_calculator._lookup_aiz_height",
            return_value=(45.0, "BIAL funnel"),
        ):
            from utils.fsi_calculator import calculate_fsi
    ...
    def test_aiz_db_error_returns_none(self):
        with patch(
            "utils.fsi_calculator._lookup_aiz_height", return_value=(None, None)
        ):
            from utils.fsi_calculator import calculate_fsi
    ...
    def test_parking_deducted_from_sellable(self):
        ...
        rec = TypologyRecommender(
            total_units=100, avg_listing_psf=7000, efficiency=0.65
        )
    ...
    def test_typology_uses_market_in_recommend(self):
        ...
        with patch("utils.fsi_calculator.calculate_fsi") as mock_calc:
            ...
            rec = TypologyRecommender(
                total_units=100, avg_listing_psf=7000, market="Devanahalli", zone="R2"
            )
            rec.recommend()
