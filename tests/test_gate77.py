...
class TestGate77:
    ...
    def test_hebbal_meets_data_floor(self):
        ...
        live_projects = [
            {
                "rera_number": f"RERA/HEB/{i:06d}",
                "project_name": f"Hebbal Project {i}",
                "data_source": "rera_karnataka_live",
                "scraped_at": "",
            }
            for i in range(200)
    ...
    def test_detail_scout_loads_session_cookie_from_checkpoint(self):
        ...
        with patch("config.checkpointer.Checkpointer", return_value=mock_cp):
            ...
            assert cookie == "test-session-value", (
                "session_cookie not loaded from checkpoint"
            )
    ...
    def test_agent_runs_has_yelahanka_scraper_row(self):
        ...
        def capture_insert(sql, **kwargs):
            ...
            if (
                ...
                and params.get("market") == "Yelahanka"
            ):
                import json

                md = json.loads(params.get("metadata", "{}"))
                assert md["fallback_triggered"] is False, (
                    "fallback_triggered must be False for live data"
                )
    ...
    def test_send_scraper_alert_callable(self):
        ...
        with patch("utils.discord_notifier.send") as mock_send:
            ...
            result = send_scraper_alert(
                "Yelahanka", "rera_karnataka", "FALLBACK_SEED", record_count=8
            )
