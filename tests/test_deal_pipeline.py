...
class TestDealPipeline:
    def _setup_db_mock(self, mock_eng, stage="loi", notes=None):
        ...
        mock_conn.execute.return_value.fetchone.side_effect = [
            ["m-uuid-1"],
            _fake_row(
                ["d-uuid", "45/2", stage, 0.82, None, None, None, notes, None, None]
            ),
        ]
        return mock_conn
    ...
    def test_patch_deal_discord_fires_on_signed(self):
        ...
        with patch.dict("os.environ", {"DASHBOARD_API_KEY": "test-key"}):
            with patch("dashboard.app_fastapi._get_sa_engine") as mock_eng:
                mock_conn = MagicMock()
                mock_eng.return_value.begin.return_value.__enter__.return_value = (
                    mock_conn
                )
                mock_eng.return_value.begin.return_value.__exit__ = MagicMock(
                    return_value=None
                )
                existing = _fake_row(
                    [DEAL_UUID, "45/2", "prospecting", 0.82, "Devanahalli"]
                )
                mock_conn.execute.return_value.fetchone.side_effect = [
                    existing,
                    _fake_row(
