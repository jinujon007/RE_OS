...
def test_seed_events_impact_score_in_range():
    ...
    for evt in events:
        ...
        assert 1 <= score <= 10, (
            f"impact_score {score} out of range [1,10] in: {evt['headline'][:50]}"
        )
...
def test_seed_events_signal_strength_valid():
    ...
    for evt in events:
        ...
        assert sig in valid, (
            f"Invalid signal_strength '{sig}' in: {evt['headline'][:50]}"
        )
...
def test_demand_signals_has_infra_pipeline_norm():
    ...
    assert hasattr(ds, "infra_pipeline_norm"), (
        "DemandSignals missing infra_pipeline_norm"
    )
