...
class TestHelpers:
    ...
    def test_fallback_debate_mention(self):
        responses = [
            {"name": "A", "verdict": "NEEDS_CORRECTION", "top_concern": "Risk"}
        ]
...
class TestMakeLLMCall:
    ...
    def test_make_llm_call_returns_verdict(self, mock_create):
        ...
        mock_agent.execute.return_value = (
            '{"verdict": "GO_ON_PLAN", "top_concern": "All good"}'
        )
    ...
    @patch(
        "crews.shareholder_review.create_shareholder_agent",
...
class TestShareholderBoardCrew:
    ...
    def test_quarterly_review_returns_dict(
        self, mock_eng, mock_save, mock_specs, mock_audit, mock_digest
    ):
    ...
    @patch("crews.shareholder_review._save_session")
    @patch("crews.shareholder_review.get_engine")
    def test_ceo_synthesis_non_empty(
        self, mock_eng, mock_save, mock_specs, mock_audit, mock_digest
    ):
        mock_digest.return_value = {
        ...
        assert result["quarter_verdict"] in (
            "GO_ON_PLAN",
            "NEEDS_CORRECTION",
            "UNDERPERFORMING",
        )
    ...
    def test_save_letter_creates_file(self, tmp_path):
        with patch("crews.shareholder_review.get_engine") as mock_eng:
            with (
                ...
                file_path = ShareholderBoardCrew.save_letter(
                    "test-id", "CEO Letter Text", "Q2-2026"
                )
