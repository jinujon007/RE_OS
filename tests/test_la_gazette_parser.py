...
def test_is_la_notification_detects_keywords():
    ...
    assert parser._is_la_notification(
        "This is a land acquisition notification under Section 4(1)"
    )
    assert parser._is_la_notification(
        "KIADB preliminary notification for acquisition of land"
    )
    assert parser._is_la_notification(
        "BMRCL Final Notification for metro corridor land"
    )
    assert not parser._is_la_notification(
        "This is a routine revenue department circular"
    )
    assert not parser._is_la_notification("")
...
def test_extract_notification_number():
    ...
    assert (
        "KIADB/LAQ/CR/2026/1234" in notif.notification_no
        or "KIADB" in notif.notification_no
    )
...
def test_extract_authority_from_text():
    ...
    for expected, text in tests:
        notif = parser._extract_notification(text)
        assert notif.authority == expected, (
            f"Failed for {expected}: got {notif.authority}"
        )
