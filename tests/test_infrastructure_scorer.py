...
class TestInfrastructureScorer:
    ...
    def test_road_distances_flag_true_when_graph_loaded(self):
        ...
        with patch("utils.infrastructure_scorer._load_graph", return_value=mock_G), \
             ...
             patch("utils.infrastructure_scorer._compute_walkability", return_value=(7.5, 35)):
            scorer = self._make_scorer()
            r = scorer.score(13.1007, 77.5963, "Yelahanka")
...
class TestWriteToDb:
    def test_write_to_db_returns_bool(self):
        from utils.infrastructure_scorer import InfrastructureScorer, InfrastructureScore
        s = InfrastructureScore(lat=13.1, lng=77.6, market="Yelahanka",
                                dist_to_nearest_metro_m=500.0)
