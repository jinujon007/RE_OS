...
class TestDML:
    ...
    def test_ingest_log_append_only_no_unique_constraint(self, tx_conn):
        ...
        assert row[0] == 2, (
            "ingest_log should allow duplicate rows (append-only, no unique constraint)"
        )

    def test_opportunity_scores_insertable(self, tx_conn):
        # Verify target market exists in seed data
        market = tx_conn.execute(
            text("SELECT id FROM micro_markets WHERE slug = 'yelahanka'")
        ).fetchone()
        assert market is not None, (
            "Seed data missing: micro_markets with slug='yelahanka'"
        )
        row = tx_conn.execute(
            text("""
    ...
    def test_deals_insertable(self, tx_conn):
        row = tx_conn.execute(
            ...
            RETURNING id
        """),
            {"n": "Test Deal", "t": "jd", "s": "evaluating"},
        ).fetchone()
        assert row is not None, "deals insert failed"

    def test_surveys_insertable(self, tx_conn):
        market = tx_conn.execute(
            text("SELECT id FROM micro_markets WHERE slug = 'devanahalli'")
        ).fetchone()
        assert market is not None, (
            "Seed data missing: micro_markets with slug='devanahalli'"
        )
        row = tx_conn.execute(
            text("""
...
class TestEdgeCases:
    def test_empty_ingest_log_market_accepted(self, tx_conn):
        ...
        row = tx_conn.execute(
            text("SELECT COUNT(*) FROM ingest_log WHERE plugin_id = 'test_empty'")
        ).fetchone()
    ...
    def test_long_plugin_id_truncation(self, tx_conn):
        ...
        row = tx_conn.execute(
            text("SELECT COUNT(*) FROM ingest_log WHERE LENGTH(plugin_id) = 100")
        ).fetchone()
...
class TestIndexSpec:
    def test_idx_rera_projects_distressed_exists(self, conn):
        ...
        assert indexes is not None, (
            "Index idx_rera_projects_distressed missing (T-709 Path 1)"
        )
    ...
    def test_idx_kaveri_reg_market_date_exists(self, conn):
        ...
        assert indexes is not None, (
            "Index idx_kaveri_reg_market_date missing (T-709 Path 3)"
        )
    ...
    def test_idx_agent_registry_spec_gin_exists(self, conn):
        ...
        assert indexes is not None, (
            "Index idx_agent_registry_spec_gin missing (T-709 Path 6)"
        )
