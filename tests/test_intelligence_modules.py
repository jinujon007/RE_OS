...
def _fake_demand_signals(**kw):
    ...
    defaults = dict(market="Devanahalli", collected_at="2026-06-03T00:00:00",
                    market_found=True, demand_signal="BULLISH", demand_score=2.5,
                    months_of_supply=8.0, signals=["Price momentum healthy"])
    defaults.update(kw)
...
class TestMarketIntel:
    ...
    def test_cache_returns_cached_on_second_call(self):
        with patch("intelligence.market_intel.validate_market",
...
class TestDemandIntel:
    ...
    def test_cache_returns_cached_on_second_call(self):
        with patch("intelligence.demand_intel.validate_market",
                   return_value=_FAKE_MARKET_INFO):
            with patch("utils.db.get_engine", return_value=_mock_engine()):
                from intelligence.demand_intel import DemandIntel
...
class TestFinancialIntel:
    ...
    def test_database_error_returns_partial(self):
        with patch("intelligence.financial_intel.validate_market",
                   return_value=_FAKE_MARKET_INFO):
            with patch("utils.fsi_calculator.calculate_fsi",
                       side_effect=Exception("FSI engine down")):
                from intelligence.financial_intel import FinancialIntel
                fe = FinancialIntel(caller="test").evaluate(
                    "Devanahalli", 43560, 5000)
...
class TestIntelRegistry:
    ...
    def test_single_module_failure_others_still_run(self):
        ...
        try:
            ...
            for attr in ("demand_signals", "land_picture",
                         "legal_picture", "financial_evaluation"):
    ...
    def test_two_modules_fail_remaining_three_ok(self):
        ...
        try:
            ...
            for attr in ("land_picture", "legal_picture",
    ...
    def test_all_five_modules_fail(self):
        mocks, patchers = _patch_all_modules(
            side_effects={attr: Exception(f"{attr} down")
                          for attr in ("market_pulse", "demand_signals",
                                       "land_picture", "legal_picture",
                                       "financial_evaluation")},
        )
        try:
            from intelligence.registry import IntelRegistry
            pkg = IntelRegistry().get_full_picture(
                "45/2", "Devanahalli", 5200, 4000, "compare")
            for attr in ("market_pulse", "demand_signals", "land_picture",
                         "legal_picture", "financial_evaluation"):
    ...
    def test_different_params_do_not_use_same_cache(self):
        ...
        try:
            ...
            p1 = reg.get_full_picture(
                "45/2", "Devanahalli", 5200, 4000, "compare")
            p2 = reg.get_full_picture(
                "46/1", "Yelahanka", 10000, 4500, "purchase")
            assert p1 is not p2
    ...
    def test_sell_psf_inferred_from_market_pulse_when_not_provided(self):
        ...
        try:
            ...
            pkg = reg.get_full_picture(
                "45/2", "Devanahalli", 5200, 0, "compare")
