FIX | database/schema.sql | Fresh-deploy Alembic collision — add alembic_version table + HEAD stamp (0011_add_agent_registry) to schema.sql. On Docker deploy: postgres init applies schema.sql → all tables + alembic_version stamped to HEAD; agents container runs alembic upgrade head → finds DB at HEAD → no-op → gunicorn starts cleanly. Previously: migration 0001 op.create_table() attempted on tables that already existed → hard crash. Maintenance contract documented inline: update stamp when adding new migrations. | Claude Code | 2026-06-01
