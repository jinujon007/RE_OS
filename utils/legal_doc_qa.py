...
class LegalDocQATool:
    ...
    def load_pdf(self, path: str) -> str:
        ...
        if file_size > _MAX_FILE_SIZE_BYTES:
            logger.warning("[LegalDocQA] File too large ({} MB), refusing: {}",
                           round(file_size / (1024*1024), 1), path)
            return ""
        if file_size == 0:
    ...
    def ask(self, question: str, context: str, api_key: str | object = _SENTINEL) -> dict | None:
        key = self._get_key() if api_key is _SENTINEL else api_key
        if not key or not context or not question:
            return None
        if len(context) < 10:
            logger.debug("[LegalDocQA] Context too short ({}/10 chars) for QA", len(context))
            return None
        ...
        for attempt in range(_HF_RETRY_ATTEMPTS):
            try:
                resp = _requests.post(
                    ...
                    json={"inputs": {"question": question[:256], "context": context[:4000]}},
                    timeout=self._TIMEOUT,
                )
                if resp.status_code in (503, 429):
                    delay = _retry_delay(attempt)
                    logger.debug("[LegalDocQA] HF API {} (attempt {}/{}), retrying in {:.1f}s",
                                 resp.status_code, attempt + 1, _HF_RETRY_ATTEMPTS, delay)
                    _time.sleep(delay)
                    continue
                if resp.status_code != 200:
                    logger.warning("[LegalDocQA] HF API {}: {}", resp.status_code, resp.text[:100])
                    return None
                data = resp.json()
                answer = data.get("answer", "") if isinstance(data, dict) else (
                    data[0].get("answer", "") if isinstance(data, list) and data else ""
                )
                confidence = data.get("score", 0.0) if isinstance(data, dict) else (
                    data[0].get("score", 0.0) if isinstance(data, list) and data else 0.0
                )
                return {"answer": answer, "confidence": round(confidence, 4), "question": question}
            except Exception as exc:
                if attempt == _HF_RETRY_ATTEMPTS - 1:
                    logger.debug("[LegalDocQA] ask error after {} attempts: {}", _HF_RETRY_ATTEMPTS, exc)
                    return None
    ...
    def run_title_checklist(self, survey_no: str, pdf_path: str) -> dict:
        ...
        if not text:
            logger.warning("[LegalDocQA] No text extracted from {} — empty checklist returned", pdf_path)
            return {
                q: {"answer": "", "confidence": 0.0, "flag": False}
                for q in self._CHECKLIST_QUESTIONS
            }

        logger.info("[LegalDocQA] Running {} title checklist questions on {} ({} chars)",
                    len(self._CHECKLIST_QUESTIONS), pdf_path, len(text))

        results = {}
        for i, (q_key, q_text) in enumerate(self._CHECKLIST_QUESTIONS.items(), 1):
            logger.debug("[LegalDocQA] [{}/{}] Asking: {}", i, len(self._CHECKLIST_QUESTIONS), q_text)
        ...
        logger.info("[LegalDocQA] Checklist complete: {}/{} questions flagged",
                    flagged, len(results))
        return results
    ...
    def _is_risk_flag(self, question_key: str, answer: str) -> bool:
        ...
        if not answer_stripped:
            return False
        negations = {"no", "none", "nil", "not", "n/a", "na", "null", "0",
                     "nothing", "never", "no record", "not found", "no entry",
                     "no data", "blank", "missing", "does not", "is not"}
