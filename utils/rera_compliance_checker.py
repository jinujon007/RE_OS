...
def check_developer_compliance(
    ...
    if not developer_name:
        ...
        return RERAComplianceResult(
            ...
            last_project_completed_at=None,
            months_since_last_project=None,
            inactive_anomalies=[],
    ...
    try:
        with get_engine().connect() as conn:
            ...
            if not dev_row:
                return RERAComplianceResult(
                    ...
                    compliance_signal="UNKNOWN",
                    notes=[
                        "Developer not found in RERA Karnataka DB — verify name or check RERA portal directly"
                    ],
                )
            ...
            """),
                {"dev_id": dev_row[0], **market_params},
            ).fetchone()
        ...
        logger.warning(
            "[RERACompliance] DB query failed for developer=%s market=%s: %s",
            developer_name,
            market,
            exc,
        )
    ...
    if total == 0:
        return RERAComplianceResult(
            ...
            compliance_signal="UNKNOWN",
            notes=[
                f"Developer '{resolved_name}' found but has no projects in "
    ...
    if delayed == 0:
        ...
        signal = "WATCH"
        notes.append(
            f"{delayed}/{total} projects delayed, avg {avg_delay}mo — within tolerable range."
        )
    else:
        signal = "RISK"
        notes.append(
            f"{delayed}/{total} projects delayed, avg {avg_delay}mo — material delay risk."
        )

    if total < 3:
        notes.append(
            "Fewer than 3 RERA projects — limited track record. Increase diligence."
        )

    if inactive_anomalies:
        notes.append(
            f"{len(inactive_anomalies)} project(s) with inactive/cancelled/suspended status — review details."
        )
    if months_since_last is not None and months_since_last > 36:
        notes.append(
            f"No completed projects in {months_since_last:.0f} months — verify developer is still active in this market."
        )

    market_tag = f" (market: {market})" if market else ""
    logger.info(
        "[RERACompliance] developer=%s%s total=%d active=%d delayed=%d signal=%s anomalies=%d last_completed=%s",
        resolved_name,
        market_tag,
        total,
        active,
...
if __name__ == "__main__":
    ...
    for name in ("Brigade", "Prestige", "NonexistentDeveloper"):
        ...
        print(
            json.dumps(
                {k: v for k, v in result.__dict__.items() if k != "inactive_anomalies"},
                indent=2,
                default=str,
            )
        )
