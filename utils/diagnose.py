...
with db.engine.connect() as conn:
    ...
    orphaned = conn.execute(
        sa.text("SELECT COUNT(*) FROM rera_projects WHERE micro_market_id IS NULL")
    ).scalar()
    ...
    for row in rows:
        ...
        print(
            f"    {row.micro_market}: {row.total_projects} projects, "
...
if os.path.exists(ckpt_dir):
    ckpts = sorted(glob.glob(f"{ckpt_dir}/*.json"), key=os.path.getmtime, reverse=True)[
        :5
    ]
