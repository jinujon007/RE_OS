...
def get_survey_facts(survey_no: str, market: Optional[str] = None) -> list[SurveyFacts]:
    ...
    with _get_conn() as conn:
        ...
        """), {"sno": survey_no, "mkt": market.lower() if market else None}).fetchall()
    return [
        SurveyFacts(
            survey_id=str(r[0]), survey_no=r[1], micro_market=r[2], village=r[3],
            total_area_acres=r[4], land_type=r[5], ownership_type=r[6],
            dc_conversion_status=r[7], khata_no=r[8], khata_type=r[9],
            encumbrance_clear=r[10] or False, rtc_owner_names=r[11],
            litigation_summary=r[12], active_deal_count=r[13] or 0,
            best_opportunity_score=r[14],
        ) for r in rows
    ]


def get_developer_health(market: Optional[str] = None, min_health: Decimal = Decimal("0")) -> list[DeveloperHealth]:
    ...
    with _get_conn() as conn:
        if market:
            ...
            """), {"market": f"%{market}%", "min_h": float(min_health)}).fetchall()
        else:
            rows = conn.execute(text("""
                SELECT developer_name, grade, health_score, health_rating,
                       latest_distress_score, distress_alert_level, total_projects,
                ...
                ORDER BY health_score ASC
            """), {"min_h": float(min_health)}).fetchall()
    return [
        DeveloperHealth(
            developer_name=r[0], grade=r[1], health_score=r[2], health_rating=r[3],
            latest_distress_score=r[4], distress_alert_level=r[5],
            total_projects=r[6] or 0, delayed_projects=r[7] or 0, delay_rate_pct=r[8],
...
def get_market_pulse(market: Optional[str] = None) -> list[MarketPulse]:
    ...
    result = [
        MarketPulse(
            micro_market=r[0], total_projects=r[1] or 0, active_projects=r[2] or 0,
            total_units=r[3] or 0, total_unsold=r[4] or 0, avg_psf_rera=r[5],
            avg_listing_psf=r[6], igr_median_psf=r[7], benchmark_psf=r[8],
            months_of_supply=r[9], supply_label=r[10] or "INSUFFICIENT_DATA",
            sentiment_label=r[11] or "NEUTRAL", listing_to_igr_gap_pct=r[12],
        ) for r in rows
    ]
...
def get_opportunity_queue(market: Optional[str] = None, min_score: Decimal = Decimal("0.4"), limit: int = 20) -> list[OpportunityQueueItem]:
    ...
    with _get_conn() as conn:
        if market:
            rows = conn.execute(text("""
                ...
                ORDER BY score DESC
                LIMIT :lim
            """), {"market": f"%{market}%", "min_s": float(min_score), "lim": limit}).fetchall()
        else:
            rows = conn.execute(text("""
                SELECT opportunity_id, survey_no, micro_market, developer_name,
                       score, irr_score, legal_risk_level, best_deal_type,
                ...
                ORDER BY score DESC
                LIMIT :lim
            """), {"min_s": float(min_score), "lim": limit}).fetchall()
    return [
        OpportunityQueueItem(
            opportunity_id=str(r[0]), survey_no=r[1], micro_market=r[2],
            developer_name=r[3], score=r[4], irr_score=r[5],
            legal_risk_level=r[6], best_deal_type=r[7], estimated_jd_irr=r[8],
            next_action=r[9], expiry_date=r[10],
            priority_label=r[11] or "COLD", market_rank=r[12] or 0,
        ) for r in rows
    ]


def get_ingest_log(plugin_id: Optional[str] = None, market: Optional[str] = None, status: Optional[str] = None, limit: int = 50) -> list[IngestLogEntry]:
    ...
    return [
        IngestLogEntry(
            id=str(r[0]), plugin_id=r[1], market=r[2], entity_type=r[3],
            status=r[4], error_message=r[5], created_at=r[6],
        ) for r in rows
    ]
...
def get_data_freshness() -> list[FreshnessReport]:
    ...
    return [
        FreshnessReport(
            source_name=r[0], last_scraped_at=r[1], record_count=r[2] or 0,
            freshness_score=r[3] or 0.0, freshness_label=r[4] or "NEVER_SCRAPED",
        ) for r in rows
    ]
