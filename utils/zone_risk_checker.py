...
def check_zone_risk(market: str, zone: str = "R2") -> ZoneRiskResult:
    ...
    try:
        with get_engine().connect() as conn:
            ...
            # Overlay constraints — try spatial join first (needs populated geometry).
            # Fallback to global scan if PostGIS/geometry unavailable.
            # SAVEPOINT ensures failed spatial query doesn't abort the transaction.
            overlays = []
            try:
                ...
                logger.debug("[ZoneRisk] SAVEPOINT management failed — overlay query may be incomplete")
    ...
    if row:
        ...
        result.setback_rear_m = float(row[5]) if row[5] is not None else None
    ...
    # Any overlay constraint is inherently a risk — flag ALL types
    risk_flags = []
    if overlays:
        seen_types = set()
