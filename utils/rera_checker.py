...
def resolve_developer_name(name: str) -> tuple[str, float]:
    ...
    try:
        ...
        with get_engine().connect() as conn:
            # Exact match first
            row = conn.execute(
                text("SELECT canonical_name, match_confidence FROM developer_aliases WHERE alias ILIKE :n LIMIT 1"),
                {"n": name},
            ).fetchone()
            if row:
            ...
            # Partial match
            row = conn.execute(
                text("SELECT canonical_name, match_confidence FROM developer_aliases WHERE :n ILIKE '%' || alias || '%' LIMIT 1"),
                {"n": name},
            ).fetchone()
            if row:
...
def check_developer_compliance_extended(
    ...
    if use_alias_resolution:
        ...
        logger.info("[DeveloperChecker] '%s' → '%s' (conf=%.2f)", developer_name, resolved_name, match_confidence)
