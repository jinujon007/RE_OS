...
class SpatialClusterAnalyzer:
    ...
    def analyze_market(self, market: str) -> SpatialAutocorrelationResult:
        ...
        try:
            ...
            for r in rows:
                ...
                if not (lon and lat):
                    continue
                avg_psf = (float(psf_min or 0) + float(psf_max or 0)) / 2 if psf_min or psf_max else 0
            ...
            try:
                ...
                logger.info("[SpatialIntel] %s: %s (I=%.3f, p=%.4f, n=%d)",
                           market, result.pattern, result.morans_i, result.p_value, result.n_projects)
    ...
    def format_for_ceo(self, result: SpatialAutocorrelationResult) -> str:
        ...
        emoji = {"clustered": "🔴", "dispersed": "🟢", "random": "🟡"}.get(result.pattern, "⚪")
        return (
