...
def _load_graph(market: str):
    ...
    if not graphml_path.exists():
        logger.warning(
            "[InfraScore] No cached graph for {} at {}", market, graphml_path
        )
        return None
    try:
        G = ox.load_graphml(str(graphml_path))
        logger.debug(
            "[InfraScore] Loaded graph {}: {} nodes, {} edges",
            graphml_path,
            G.number_of_nodes(),
            G.number_of_edges(),
        )
...
def _road_distance_m(G, from_node: int, to_lat: float, to_lng: float) -> float | None:
    ...
    try:
        ...
        if to_node == from_node:
            return 0.0
        length = ox.distance.shortest_path_length(
...
class InfrastructureScorer:
    ...
    def score(self, lat: float, lng: float, market: str) -> InfrastructureScore:
        ...
        if metro_coords:
            ...
            result.dist_to_nearest_metro_m = round(
                _haversine_m(lat, lng, mlat, mlng), 1
            )
        ...
        logger.info(
            "[InfraScore] {} ({:.4f}, {:.4f}): metro={}m NH44={}m BIAL={}km CBD={}km walk={}",
            market_safe,
            lat,
            lng,
    ...
    def score_location(
        self, lat: float, lon: float, market: str
    ) -> "AccessibilityScore":
    ...
    def score_survey(self, survey_no: str, market: str) -> "AccessibilityScore":
        ...
        try:
            ...
            if row and row[0] and row[1]:
                return InfrastructureScorer._pandana_score_location(
                    float(row[1]), float(row[0]), market
                )
        except Exception as exc:
    ...
    @staticmethod
    def _pandana_score_location(
        lat: float, lon: float, market: str
    ) -> "AccessibilityScore":
        ...
        try:
            ...
            G = ox.graph_from_point(
                (lat, lon), dist=5000, network_type="drive", simplify=True
            )
            ...
            pois = ox.geometries_from_point(
                (lat, lon), tags={"amenity": amenities}, dist=5000
            )
            ...
            logger.debug(
                "[InfraScore] Pandana score for (%s, %s): overall=%.2f",
                lat,
                lon,
                result.overall,
            )
            ...
            logger.warning(
                "[InfraScore] Pandana scoring failed for (%s, %s): %s", lat, lon, exc
            )
        return result
    ...
    def write_to_db(self, result: InfrastructureScore) -> bool:
        ...
        try:
            ...
            logger.warning(
                "[InfraScore] DB write failed for {}: {}", result.market, exc
            )
            return False
