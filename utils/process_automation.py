...
LLM_RETRY_BASE_DELAY_S = float(
    os.environ.get("PROCESS_AUTOMATION_RETRY_DELAY_S", "1.0")
)
...
def run_with_timeout(
    ...
    with ThreadPoolExecutor(max_workers=_THREAD_POOL_MAX_WORKERS) as pool:
        ...
        try:
            ...
            logger.warning(
                "[Exec] {ctx} timed out after {t}s", ctx=context, t=timeout_s
            )
            return None
        except Exception as exc:
            logger.warning(
                "[Exec] {ctx} failed: {err}", ctx=context, err=str(exc)[:200]
            )
            return None
...
_LLM_RESPONSE_MAX_BYTES = int(
    os.environ.get("PROCESS_AUTOMATION_RESPONSE_MAX", "100000")
)
_JSON_EXTRACT_MAX_CHARS = int(
    os.environ.get("PROCESS_AUTOMATION_JSON_EXTRACT_MAX", "20000")
)


def safe_extract_json(
    raw: str, max_chars: int = _JSON_EXTRACT_MAX_CHARS
) -> dict[str, Any] | None:
    import json
