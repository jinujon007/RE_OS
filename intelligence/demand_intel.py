...
class DemandSignals:
    ...
    kaveri_monthly_approvals: float | None = (
        None  # official approvals/month (6-month avg)
    )
...
class DemandIntel:
    ...
    def _compute_demand_score_v2(self, ds: DemandSignals):
        ...
        if ds.kaveri_velocity_ratio is not None:
            components.append(
                min(ds.kaveri_velocity_ratio / self._KAVERI_VELOCITY_CEILING, 1.0)
            )
            weights.append(0.18)
        ...
        if components:
            ...
            ds.demand_score_v2 = round(
                max(0.0, min((ds.demand_score + 5.0) / 10.0, 1.0)), 4
            )
    ...
    def _load_listing_trends(self, conn, ds: DemandSignals, mi: dict):
        ...
        if (
            ...
            and row30
            and row90_prior[1]
            and int(row90_prior[1]) >= _MIN_SAMPLES_FOR_TREND
            and row30[1]
            and int(row30[1]) >= _MIN_SAMPLES_FOR_TREND
        ):
    ...
    def _load_kaveri_registration_velocity(self, conn, ds: DemandSignals, mi: dict):
        ...
        try:
            ...
            vol = GazetteParser().scrape_registration_velocity_signal(
                mi["name"], from_date, to_date
            )
            if not vol:
                return
            ...
            logger.debug(
                "[DemandIntel] Kaveri registration velocity failed for {}: {}",
                mi["name"],
                exc,
            )
    ...
    def _load_gcc_signal(self, ds: DemandSignals, mi: dict):
        ...
        try:
            ...
            logger.debug(
                "[DemandIntel] gcc signal load failed for {}: {}", mi["name"], exc
            )
    ...
    def _load_govt_pipeline_signal(self, ds: DemandSignals, mi: dict):
        ...
        try:
            ...
            logger.debug(
                "[DemandIntel] govt pipeline signal load failed for {}: {}",
                mi["name"],
                exc,
            )
