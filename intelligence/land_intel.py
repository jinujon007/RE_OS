...
class LandIntel:
    ...
    def _load_regulatory_data(self, conn, pic: LandPicture, mi: dict):
        ...
        if row and row[0]:
            logger.info(
                "[{}] Using regulatory zone {} for {} (no market FK in v1 schema)",
                self._caller,
                row[0],
                mi.get("name", "?"),
            )
    ...
    def _load_infrastructure(self, conn, pic: LandPicture, mi: dict):
        ...
        try:
            ...
            infra.accessibility_score = compute_market_accessibility(
                mi["name"], conn=conn
            )
        except Exception as exc:
            logger.warning(
                "[{}] accessibility_score compute failed for {}: {}",
                self._caller,
                mi.get("name", "?"),
                exc,
            )
    ...
    def _assess_risks(self, pic: LandPicture):
        if pic.overlay_count > 2:
            ...
            pic.flags.append(
                f"{pic.overlay_count} overlay constraints — detailed survey needed"
            )
        elif pic.overlay_count > 0:
            pic.development_readiness = "PARTIAL"
            pic.flags.append(
                f"{pic.overlay_count} overlay constraint(s) — verify exact buffer impact"
            )
        elif pic.far:
        ...
        if pic.meets_bda_minimum and pic.green_pct:
            pic.flags.append(
                f"BDA green coverage minimum met ({pic.green_pct:.0f}% ≥ 15%)"
            )
        elif pic.green_pct is not None:
            pic.flags.append(
                f"Below BDA 15% green minimum ({pic.green_pct:.0f}%) — adjust site plan"
            )
        ...
        if not pic.far:
            pic.flags.append(
                "No FAR data — manual BDA/BMRDA zone verification required"
            )
            pic.development_readiness = "UNKNOWN"

        if pic.flood_risk == "ALERT":
            pic.flags.append(
                "Lake buffer zone detected — flood risk, construction restrictions apply"
            )
            pic.development_readiness = "CONSTRAINED"
        ...
        if pic.infrastructure:
            ...
            if acc < 0.3:
                pic.flags.append(
                    f"Low transit accessibility (score={acc:.2f}) — may limit demand pool"
                )
            elif acc < 0.5:
                pic.flags.append(
                    f"Moderate transit accessibility (score={acc:.2f}) — monitor infra pipeline"
                )
