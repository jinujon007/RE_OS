...
class TitleRiskFlag:
    ...
    def __str__(self) -> str:
        icon = {"CLEAR": "✓", "WARNING": "△", "RISK": "✗", "UNKNOWN": "?"}.get(
            self.status, "?"
        )
        ...
        try:
            ...
            if pdf_path:
                try:
                    ...
                    logger.warning(
                        "[{}] PDF QA failed for {}: {}", self._caller, pdf_path, qa_exc
                    )

        except Exception as exc:
            logger.warning(
                "[{}] get_survey_picture({}, {}) failed: {}",
                self._caller,
                s,
                mi["name"],
                exc,
            )
            pic.title_risk_flags.append(
                TitleRiskFlag(
                    flag="db_error",
                    status="UNKNOWN",
    ...
    def _check_guidance_value(self, conn, pic: LegalPicture, mi: dict):
        ...
        if row and row[0]:
            ...
            if market_psf and market_psf > 0:
                gap = round(
                    (market_psf - pic.guidance_value_psf)
                    / pic.guidance_value_psf
                    * 100,
                    1,
                )
