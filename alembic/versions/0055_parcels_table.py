...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "village", "survey_no", name="uq_parcels_village_survey_no"
        ),
    )

    op.create_index(
        "idx_parcels_village_survey_no", "parcels", ["village", "survey_no"]
    )
    ...
    op.add_column("rera_projects", sa.Column("parcel_id", sa.UUID(), nullable=True))
    ...
    op.add_column(
        "registered_transactions", sa.Column("parcel_id", sa.UUID(), nullable=True)
    )
    op.create_foreign_key(
    ...
    op.create_index(
        "idx_registered_transactions_parcel_id",
        "registered_transactions",
        ["parcel_id"],
    )

    op.add_column(
        "kaveri_registrations", sa.Column("parcel_id", sa.UUID(), nullable=True)
    )
    op.create_foreign_key(
    ...
    op.create_index(
        "idx_kaveri_registrations_parcel_id", "kaveri_registrations", ["parcel_id"]
    )


def downgrade() -> None:
    op.drop_index(
        "idx_kaveri_registrations_parcel_id", table_name="kaveri_registrations"
    )
    op.drop_constraint(
        "fk_kaveri_registrations_parcel_id", "kaveri_registrations", type_="foreignkey"
    )
    op.drop_column("kaveri_registrations", "parcel_id")

    op.drop_index(
        "idx_registered_transactions_parcel_id", table_name="registered_transactions"
    )
    op.drop_constraint(
        "fk_registered_transactions_parcel_id",
        "registered_transactions",
    ...
    op.drop_constraint(
        "fk_rera_projects_parcel_id", "rera_projects", type_="foreignkey"
    )
