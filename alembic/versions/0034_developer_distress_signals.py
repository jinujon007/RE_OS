...
def upgrade():
    op.create_table(
        ...
        sa.Column("stall_count", sa.Integer(), server_default=sa.text("0"), nullable=False),
        sa.Column("stall_ratio", sa.Float(), server_default=sa.text("0.0"), nullable=False),
        sa.Column("mention_count", sa.Integer(), server_default=sa.text("0"), nullable=False),
        sa.Column("distress_score", sa.Float(), server_default=sa.text("0.0"), nullable=False),
        sa.Column("detected_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()"), nullable=False),
        sa.Column("ingest_log_id", sa.UUID(), sa.ForeignKey("ingest_log.id"), nullable=True),
    )
...
def downgrade():
    op.drop_index("idx_developer_distress_market_detected", table_name="developer_distress_signals")
    op.drop_constraint("uq_developer_distress_signal", "developer_distress_signals", type_="unique")
    op.drop_constraint("ck_developer_distress_signal_type", "developer_distress_signals", type_="check")
