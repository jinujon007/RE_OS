...
def upgrade():
    ...
    op.create_check_constraint(
        "ck_landowner_contact_type",
        "landowner_contacts",
        "contact_type IN ('primary','agent','legal_heir','power_of_attorney')",
    )
    op.create_check_constraint(
        "ck_landowner_approach_status",
        "landowner_contacts",
        "approach_status IN ('cold','warm','meeting_done','mou','loi','closed_won','closed_lost')",
    )
    op.create_index(
        "idx_landowner_contacts_survey_market",
        "landowner_contacts",
        ["survey_no", "market"],
    )
    op.create_index(
        "idx_landowner_contacts_status", "landowner_contacts", ["approach_status"]
    )
...
def downgrade():
    ...
    op.drop_index(
        "idx_landowner_contacts_survey_market", table_name="landowner_contacts"
    )
    op.drop_constraint(
        "ck_landowner_approach_status", "landowner_contacts", type_="check"
    )
