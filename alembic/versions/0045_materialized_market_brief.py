"""Create v_market_brief_mat materialized view (T-1093 GATE-82).

An exact replica of v_market_brief (from migration 0023) as a materialized view
with a unique index on micro_market, enabling non-blocking CONCURRENT refreshes.
Eliminates 4-count-subquery scans on every query.

Migration chain:
    0043_rera_projects_bhoomi_checked_at -> 0045_materialized_market_brief

NOTE: This migration skips revision 0044. If Sprint 81 (T-1088) later adds
...
revision: str = "0045_materialized_market_brief"
down_revision: Union[str, None] = "0043_rera_projects_bhoomi_checked_at"
branch_labels: Union[str, None] = None
depends_on: Union[str, None] = None

_MAT_VIEW_SQL = """
CREATE MATERIALIZED VIEW IF NOT EXISTS v_market_brief_mat AS
...
_DROP_SQL = "DROP MATERIALIZED VIEW IF EXISTS v_market_brief_mat CASCADE"

_IDX_SQL = (
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_v_market_brief_mat_market "
    "ON v_market_brief_mat (micro_market)"
)

_REFRESH_SQL = "REFRESH MATERIALIZED VIEW v_market_brief_mat"


def upgrade():
    op.execute(_MAT_VIEW_SQL)
    op.execute(_IDX_SQL)
