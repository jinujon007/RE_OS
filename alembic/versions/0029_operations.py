...
def upgrade():
    op.create_table(
        ...
        sa.Column(
            "status", sa.VARCHAR(30), server_default=sa.text("'lead'"), nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["source_deal_id"], ["deal_pipeline.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["source_board_session_id"],
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.VARCHAR(20), server_default=sa.text("'todo'"), nullable=True
        ),
    ...
    op.create_index(
        "idx_project_tasks_project_status", "project_tasks", ["project_id", "status"]
    )
