...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("source", sa.Text(), nullable=False, server_default=sa.text("'naukri_search'")),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=True),
        sa.UniqueConstraint("employer", "location", "snapshot_date", name="uq_gcc_snapshot_employer_loc_date"),
    )
    ...
    op.add_column("gcc_events",
        sa.Column("data_source", sa.Text(), nullable=True, server_default=sa.text("'seed'"))
    )
