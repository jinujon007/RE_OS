...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "micro_market_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("micro_markets.id"),
            nullable=False,
        ),
    ...
    op.create_index(
        "idx_surveys_geom", "surveys", [sa.text("geom")], postgresql_using="gist"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "survey_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("surveys.id")
        ),
        sa.Column("survey_no", sa.String(100), nullable=False),
        sa.Column(
            "micro_market_id",
    ...
    op.create_table(
        ...
        sa.Column(
            "project_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("rera_projects.id"),
        ),
        ...
        sa.Column(
            "detected_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")
        ),
        sa.Column(
    ...
    op.create_index(
        "idx_distressed_score", "distressed_opps", [sa.text("distress_score DESC")]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "computed_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")
        ),
        sa.Column(
            "updated_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")
        ),
    ...
    op.create_index(
        "idx_dev_health_score", "developer_health", [sa.text("health_score DESC")]
    )
    ...
    op.create_index(
        "idx_demand_market",
        "demand_signals",
        ["micro_market_id", sa.text("signal_date DESC")],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "session_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("board_sessions.session_id"),
        ),
        sa.Column(
            "created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")
        ),
    ...
    op.create_index(
        "idx_compliance_entity", "compliance_log", ["entity_type", "entity_id"]
    )
    ...
    op.create_index(
        "idx_opp_scores_market_score",
        "opportunity_scores",
        ["micro_market_id", sa.text("score DESC")],
    )
    ...
    op.create_index(
        "idx_rera_projects_distressed",
        "rera_projects",
        ["micro_market_id", "project_status", "possession_date"],
    )
    op.create_index(
        "idx_rera_projects_active_inv",
        "rera_projects",
        ["micro_market_id", "is_active"],
        postgresql_where=sa.text("is_active = TRUE"),
    )
    op.create_index(
        "idx_kaveri_reg_market_date",
        "kaveri_registrations",
        ["micro_market_id", sa.text("transaction_date DESC")],
    )
    op.create_index(
        "idx_igr_market_date_v2",
        "igr_transactions",
        ["micro_market_id", sa.text("registration_date DESC")],
    )
    op.create_index(
        "idx_agent_registry_spec_gin",
        "agent_registry",
        [sa.text("spec")],
...
def downgrade() -> None:
    ...
    for idx_name in (
        "idx_opp_scores_expiry",
        "idx_opp_scores_active",
        "idx_opp_scores_survey",
        "idx_opp_scores_market_score",
    ):
    ...
    for idx_name in (
        "idx_distressed_alert",
        "idx_distressed_market",
        "idx_distressed_score",
        "idx_distressed_dev",
    ):
    ...
    for idx_name in (
        "idx_litigations_status",
        "idx_litigations_market",
        "idx_litigations_survey_no",
        "idx_litigations_survey",
    ):
