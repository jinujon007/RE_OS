...
def upgrade():
    op.create_table(
        ...
        sa.Column("shareholder_responses", postgresql.JSONB(), server_default=sa.text("'[]'::jsonb"), nullable=True),
    ...
    op.create_index("idx_shareholder_sessions_type_quarter", "shareholder_sessions", ["session_type", "quarter"])
    op.create_index("idx_shareholder_sessions_status", "shareholder_sessions", ["status"])


def downgrade():
    op.drop_index("idx_shareholder_sessions_status", table_name="shareholder_sessions")
    op.drop_index("idx_shareholder_sessions_type_quarter", table_name="shareholder_sessions")
    op.drop_constraint("ck_shareholder_sessions_status", "shareholder_sessions", type_="check")
    op.drop_constraint("ck_shareholder_sessions_session_type", "shareholder_sessions", type_="check")
