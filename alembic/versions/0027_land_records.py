...
def upgrade():
    op.create_table(
        ...
        sa.Column("encumbrances", JSONB(), server_default=sa.text("'[]'::jsonb"), nullable=True),
        sa.Column("bhoomi_status", sa.VARCHAR(30), server_default=sa.text("'live'"),
                  nullable=True),
        sa.Column("bhoomi_fetched_at", sa.TIMESTAMP(), server_default=sa.text("NOW()"), nullable=True),
    )
    op.create_check_constraint("ck_land_records_land_nature",
        "land_records",
        "land_nature IN ('agricultural','converted','revenue','notional','unknown')",
    )
    op.create_check_constraint("ck_land_records_bhoomi_status",
        "land_records",
        "bhoomi_status IN ('live','unavailable','partial')",
    )
    op.create_index("idx_land_records_survey", "land_records", ["survey_no"])
    op.create_index("idx_land_records_survey_market", "land_records", ["survey_no", "market"])
