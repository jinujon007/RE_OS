...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_assembly_signals_status_alerted",
        "assembly_signals",
        ["status", "discord_alerted"],
    )
    op.create_index("idx_assembly_signals_village", "assembly_signals", ["village"])
    op.create_index(
        "idx_assembly_signals_buyer", "assembly_signals", ["buyer_name_norm"]
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "ck_assembly_signals_confidence", "assembly_signals", type_="check"
    )
    op.drop_constraint(
        "ck_assembly_signals_parcel_count", "assembly_signals", type_="check"
    )
    op.drop_constraint(
        "uq_assembly_signals_buyer_village", "assembly_signals", type_="unique"
    )
